...
- [ ] **Phase 0: 盤點與凍結** -- 釐清 active/legacy 路徑，建立只改 active 副本的明文規範
- [ ] **Phase 1: 真相層級治理** -- 文件與程式回應統一三層分級，消除 state 過度信任殘留
- [ ] **Phase 2: 交易保險絲收斂** -- sizing policy + pre-flight gate 單一路徑化，submit 後強制落地驗證
- [ ] **Phase 3: 持倉交易票據 UI** -- drawer 展開票據、preview/confirm/submit 三段式、維持人工確認
...
**Goal**: 文件與程式回應統一採用三層分級，消除所有「state=唯一真相源」殘留說法
**Depends on**: Phase 0
**Requirements**: TRUTH-01, TRUTH-02, TRUTH-03, TRUTH-04, GIT-01, GIT-02
**Success Criteria** (what must be TRUE):
  1. 所有對查單回應（文件與程式）均標示資訊來源層級：live API 直接證據 > live 無法確認 > 次級資訊
...
**Goal**: sizing policy 與 pre-flight gate 成為單一不可繞過路徑，submit 後必須進入落地驗證流程
**Depends on**: Phase 1
**Requirements**: FUSE-01, FUSE-02, FUSE-03, FUSE-04, FUSE-05, GIT-01, GIT-02
**Success Criteria** (what must be TRUE):
...
**Goal**: 持倉主列保持乾淨，交易票據在展開層操作，preview/confirm/submit 三段式且不容許預覽即送單
**Depends on**: Phase 2
**Requirements**: TICKET-01, TICKET-02, TICKET-03, TICKET-04, TICKET-05, GIT-01, GIT-02
**Success Criteria** (what must be TRUE):
