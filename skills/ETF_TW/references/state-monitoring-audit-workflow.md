...
### Phase A2：Agent markdown 設定鏈路審計
當使用者要求檢視 etf_master agent 的 md 設定、技能調用、wiki 知識庫、dashboard 對齊時，除了讀 SOUL.md / CLAUDE.md / README.md，必須做實機對照，不可只看文件：
- `hermes config path && hermes profile list`：確認 active profile 與持久模型設定；注意當前 session 臨時切到 gpt-5.5 不代表 `config.yaml` 已同步。
- 讀 `SOUL.md`：確認策略抬頭、Intent-Skill Mapping、ETF_TW / stock-analysis-tw / stock-market-pro-tw / taiwan-finance 分工。
- 讀 `CLAUDE.md` / `README.md`：確認 state 架構、AI Decision Bridge、wiki_context、dashboard port、cron 敘述是否一致。
- 讀 `instances/etf_master/state/strategy_link.json` + `agent_summary.json`：回報目前 `[目前投資策略:XXX, 情境覆蓋:YYY]` 是否可由 state 取得。
- 檢查 dashboard runtime port：官方 etf_master port 應是 5055；若 5055 closed 但 5050/5051 open，判為殘留舊 dashboard，文件可能正確但 runtime 錯位。
- 檢查 `generate_ai_decision_request.py` 與現有 `ai_decision_request.json`：確認 `wiki_context` 不是只有程式碼接線，而是 live payload 內 `market_view` / `risk_signal` / `investment_strategies` / `undervalued_ranking` 實際非空。
- 檢查相關技能 SKILL.md 是否存在：`stock-analysis-tw`、`stock-market-pro-tw`、`taiwan-finance`。
- 特別搜尋過時查單規則：若 SOUL.md / CLAUDE.md 仍要求 `list_trades()` 當正式驗證，需標為 P0 文件風險，因 Shioaji 正確查委託/成交應優先 `api.order_deal_records(api.stock_account, timeout=10000)`，持倉用 `adapter.get_positions(...)`。
...
### Phase F2：dashboard 現金 / 追蹤數要顯示交割安全金額
當使用者要求 dashboard 在刷新時自動顯示「現金 / 交割安全金額」，正確鏈路是：
1. 先確認使用者常按的刷新入口是哪一個；目前 dashboard 主按鈕「一鍵同步與全量分析 (Full Sync)」鏈路是 `overview.html` 的 `runFullPipelineSync()` → `POST /api/decision/full-pipeline` → `_run_full_pipeline_helper()` → 先跑 `scripts/sync_live_state.py`。一般 `POST /api/refresh` 也會經 `refresh_monitoring_state.py` 在 live-ready 模式下跑 `sync_live_state.py`。
2. 不要只改前端；先改 `scripts/sync_live_state.py`，讓 Full Sync / refresh 觸發 live sync 時一併查交割款。
2. live-ready 模式下沿用既有 broker adapter 登入，不新增獨立憑證讀取流程；在 adapter authenticated 後使用 `adapter.api.settlements(adapter.stock_account)`。
3. 不要用 `api.list_settlements(...)`；實測會報 `ValueError('dictionary update sequence element #0 has length 6; 2 is required')`。正確方法是 `api.settlements(api.stock_account)`。
4. 在 `account_snapshot.json` 增加欄位：
   - `cash`：永豐帳面現金
   - `settlements`：`[{date, T, amount, source}]`
   - `future_settlement_net_t1_t2`：只加總 T=1/T=2 淨額；T=0 已屬當日現金/當日扣補，不納入未來安全額
   - `settlement_safe_cash`：`cash + future_settlement_net_t1_t2`
   - `settlement_safe_cash_floor`：給 UI 顯示用，負值歸零但保留原始負值供風控判讀
   - `settlements_error`：查詢失敗時保留錯誤，不靜默當 0
5. 前端 `dashboard/templates/overview.html` 的 `card_cash` 顯示兩個主數字：
   - 現金：`account.cash`
   - 交割安全：`account.settlement_safe_cash`
   並可附帶 `T+1/T+2 淨額` 與錯誤訊息。
6. 加測試到 `tests/test_sync_live_state.py`：確認只用 T=1/T=2 計算交割安全額，T=0 不納入；負值時 `settlement_safe_cash` 保留負數、`settlement_safe_cash_floor` 歸零。
7. 驗證必須實機：重啟 5055 dashboard → `POST /api/refresh` → `GET /api/overview` → 檢查 HTML 是否真的含「交割安全 NT$ ...」。不可只看 template diff 就宣稱完成。
