...
## Step 2.5：AGENT_ID 顯式注入契約（新安裝必備）
當使用者明確要保留 multi-instance（每人可自訂 agent 名稱）時，這一步是**必做**，不能只口頭建議。

### 目標
- 消除 `AGENT_ID missing; defaulting instance_id=etf_master` 警告
- 避免 fallback 造成跨 instance state 汙染
- 讓「新裝技能石」開箱即有完整設定

### 必改範圍（文件 + 腳本同時）
1. **文件層（安裝契約）**
   - `SKILL.md`：新增「多 instance 啟動契約」
   - `INSTALL.md`：新增 AGENT_ID 首次設定（臨時 + 永久）
   - `BOOT.md` / `README.md`：所有啟動範例帶 `AGENT_ID=<instance_id>`
2. **腳本層（入口契約）**
   - `scripts/start_dashboard.sh`：啟動前顯式注入 `AGENT_ID=${AGENT_ID:-etf_master}`
   - 任何啟動/排程入口腳本都要沿用同契約（dashboard/cron/task runner）
   - `etf_core/context.py` 維持 `AGENT_ID` 優先，`OPENCLAW_AGENT_NAME` 僅 legacy fallback
3. **文案契約（去遺毒）**
   - 明確標註：`AGENT_ID` 是 Hermes 現行標準；`OPENCLAW_AGENT_NAME` 只做向後相容
   - usage 範例若還寫 `OPENCLAW_AGENT_NAME=...`，改成 `AGENT_ID=...` 或雙寫（主副清楚）

### 驗收標準（必做）
- 設定 `AGENT_ID` 後，`etf_tw.py` / dashboard / cron 入口不再出現 defaulting 警告
- 切換不同 `AGENT_ID`，state 必須落在各自 `instances/<id>/state/` 並互不污染
- 搜索文件與關鍵腳本，確認「主敘述」皆以 AGENT_ID 為主（legacy 僅註記）

### 驗證矩陣（建議照跑）
用 `.venv/bin/python3 -c` 直接驗證 `context.get_instance_id()` 的優先序：
1. **no_env**：`AGENT_ID` 與 `OPENCLAW_AGENT_NAME` 都清空 → 應回 `etf_master`，且只出現一次 warning
2. **legacy_only**：只設 `OPENCLAW_AGENT_NAME=legacy_id` → 應回 `legacy_id`，無 warning
3. **both_set**：同時設 `AGENT_ID=primary_id` 與 `OPENCLAW_AGENT_NAME=legacy_id` → 應回 `primary_id`（確認 AGENT_ID 優先）

範例檢查指令（功能面）：
- `AGENT_ID=etf_master .venv/bin/python3 scripts/etf_tw.py search 0050` 應 exit code 0，且不再出現 defaulting 警告
- `zsh -n scripts/start_dashboard.sh` 應通過（入口注入不可破壞 shell 語法）
...
## 補充：核實「他人作業紀錄/聊天貼文」的 5 步驟（實戰）
當使用者貼上 Claude/Codex 的作業紀錄，要求「核實是否真的做過」時，避免只看文字敘述，必須做以下交叉驗證：

1. **先定位真實 repo 副本**
   - 以 `.git/config` 的 remote url 反查 repo 根目錄（例如 `tutumomo/etf_master.git`）
   - 避免在錯副本（或非 git 目錄）做驗證

2. **核對 commit 真假與內容範圍**
   - `git show --no-patch <hash>` 確認 commit 存在
   - `git show --name-only/--stat <hash>` 檢查實際改檔清單與數量，防止「口頭說只改 docs，實際有 code 變更」

3. **逐檔驗證關鍵宣稱**
   - 直接檢查被宣稱建立/修改的檔案是否存在與內容是否對齊
   - 特別注意路徑誤述：例如 worldmonitor jobs 實際在 `cron/jobs.json`，不是 `skills/ETF_TW/cron/jobs.json`

4. **核對 remote 同步狀態**
   - 比對 `local main` vs `origin/main` hash
   - 比對 tag（如 `v1.4.0`）本地與 remote 是否一致

5. **回報時分成「屬實 / 差異」兩欄**
   - 屬實：哪些敘述已被 git 與檔案證實
   - 差異：哪些是路徑錯、範圍誤述、或描述不精確
   - 這樣可同時維持信任與可審核性，不把小錯誤誇大成否定全部成果

6. **加做 GitHub 治理完整性檢查（避免「能追 commit 但審計鏈斷裂」）**
   - `gh api repos/<owner>/<repo>/branches/main --jq .protected`：確認 main 是否受保護（若 false，記為風險）
   - `gh pr list --state merged --limit 10 --json number`：確認近期是否走 PR 流程（若長期為空且頻繁直推，記為風險）
   - `git cat-file -t <tag>`：確認 release tag 類型（`commit`=lightweight，審計強度較低；`tag`=annotated）
   - 對「commit 訊息 vs 實際改動」做一致性檢查：`git show --stat <hash>`，避免 `docs(...)` 但包含 code/test/wiki 的語意誤導
