...
PORT="5057"
LOG_DIR="$ROOT/logs"
STARTUP_LOG="$LOG_DIR/dashboard-startup-etf_son.log"

# Override instance routing
export AGENT_ID="etf_son"
...
if [[ -n "$LISTEN_LINE" ]]; then
  ...
  if [[ "$CMDLINE" == *"uvicorn"* && "$CMDLINE" == *"dashboard.app:app"* ]]; then
    log "etf_son dashboard already running on ${HOST}:${PORT} (pid=$PID); exiting"
    exit 0
  fi
...
log "starting etf_son dashboard on ${HOST}:${PORT}"
