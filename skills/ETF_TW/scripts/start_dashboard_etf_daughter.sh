...
PORT="5058"
LOG_DIR="$ROOT/logs"
STARTUP_LOG="$LOG_DIR/dashboard-startup-etf_daughter.log"

# Override instance routing
export AGENT_ID="etf_daughter"
...
if [[ -n "$LISTEN_LINE" ]]; then
  ...
  if [[ "$CMDLINE" == *"uvicorn"* && "$CMDLINE" == *"dashboard.app:app"* ]]; then
    log "etf_daughter dashboard already running on ${HOST}:${PORT} (pid=$PID); exiting"
    exit 0
  fi
...
log "starting etf_daughter dashboard on ${HOST}:${PORT}"
