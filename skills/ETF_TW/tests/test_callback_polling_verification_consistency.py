...
def test_callback_then_polling_preserves_verification_metadata():
    ...
    assert rows[0]["broker_order_id"] == "43e14cbd"
    assert rows[0]["broker_status"] == "submitted"


def test_polling_then_callback_preserves_verification_metadata():
    polling_row = {
        "order_id": "43e14cbd",
        "symbol": "00922",
        "action": "buy",
    ...
    rows = orders_module.merge_open_orders([], polling_row)

    callback_row = callback_module.event_payload_to_order_row("status_update", {
        "order_id": "43e14cbd",
        "status": "submitted",
        "symbol": "00922",
        "action": "buy",
    ...
    rows = orders_module.merge_open_orders(rows, callback_row)
    assert len(rows) == 1
    assert rows[0]["verified"] is True
    assert rows[0]["broker_order_id"] == "43e14cbd"
    assert rows[0]["broker_status"] == "submitted"
