...
```bash
cd ~/.openclaw/skills/ETF_TW && .venv/bin/python3 -c "
...
路徑：`~/.openclaw/skills/ETF_TW/state_legacy_compat_link/orders_open.json`
...
```bash
# 手动执行一次确认脚本工作正常
cd ~/.openclaw/skills/ETF_TW && .venv/bin/python3 ~/.hermes/scripts/order_monitor.py
```
...
| 問題 | 原因 | 解法 |
|------|------|------|
| `Invalid character '*'` | API Key/Secret 有 `*` 遮罩字元 | 從 `~/.openclaw/skills/ETF_TW/private/.env` 讀取真實金鑰 |
